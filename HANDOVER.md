...
- Easter eggs: `easteregg.html` (Yoshi egg, 3 taps to crack, girlfriend portrait `assets/egg-face.png` + "Oh Behbi'"; tapping her opens `assets/egg-original.jpg`, the full group photo). Fake Apache-style listings at `/files/` (root), `/assets/`, `/qr/` — regenerate `assets/index.html` etc. if files change (script was inline in the session; trivial to redo by hand). Hints: `robots.txt`, an HTML comment in `index.html`, a console.log. Source photos `girlfriend_*.jpg` are gitignored. **HANDOVER.md itself is served publicly** and is deliberately NOT listed anywhere — it spoils the whole gift.
