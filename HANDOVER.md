...
- **URL fiddling is neutralised**: the current screen is remembered in sessionStorage (`sebi30-route`) and restored on any load, and hand-typed hashes to unvisited screens are reverted. Deep links / previews therefore need `?dev` (e.g. `index.html?dev#kart`, `index.html?dev&iq=4#iq-score`, `index.html?dev&seen#choose`). A fresh QR scan opens a new tab = new session = clean start.
